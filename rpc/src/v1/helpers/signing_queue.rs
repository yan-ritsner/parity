...
pub struct ConfirmationReceiver {
	id: U256,
	receiver: future::Shared<oneshot::Receiver<ConfirmationResult>>,
}
...
impl ConfirmationReceiver {
	...
	pub fn receiver(&self) -> future::Shared<oneshot::Receiver<ConfirmationResult>> {
		self.receiver.clone()
	}
}
...
mod test {
	...
	fn should_wait_for_hash() {
		...
		let handle = thread::spawn(move || {
			...
			v.receiver().wait().expect("Confirmation should be passed through the channel")
		});
		...
		assert_eq!(confirmation.clone(), ConfirmationResult::Confirmed(Ok(ConfirmationResponse::SendTransaction(1.into()))));
	}
